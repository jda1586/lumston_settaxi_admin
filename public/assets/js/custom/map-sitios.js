...
var handleGoogleMapSetting = function () {
    ...
    function initialize() {
        var defaultLat = parseFloat($("#lat").val());
        var defaultLng = parseFloat($("#lng").val());
        var mapOptions = {
            zoom: 13,
        ...
        var siteMarker = new google.maps.Marker({
            ...
            map: mapDefault, draggable: true
        });
        google.maps.event.addListener(siteMarker, 'dragend', function () {
            $("#lat").val(siteMarker.getPosition().lat());
            $("#lng").val(siteMarker.getPosition().lng());
        });
    }
